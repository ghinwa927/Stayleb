    ...
    import("@/services/api").then(({ apiFetch }) => {
      apiFetch("/rules")
        .then((rs: any[]) => {
          ...
          rs.forEach((r: any) => (m[r.id] = { name: r.name, description: r.description }));
          setRulesMap(m);
        })
        .catch(() => {});
    });
  ...
  if (loading) {
    return (
      <div className="w-full min-h-[60vh] flex items-center justify-center bg-surface">
        <div className="flex flex-col items-center gap-3"><div className="w-8 h-8 border-4 border-primary/20 border-t-primary rounded-full animate-spin" /><span className="text-sm text-primary">Loading preview…</span></div>
      </div>
    );
  }
  if (error || !property) {
    return (
      <div className="w-full min-h-[60vh] flex items-center justify-center bg-surface p-6">
        <div className="max-w-md w-full p-6 rounded-xl bg-surface-container-lowest shadow-sm border border-error/20 text-center">
          ...
          <h2 className="font-title-md text-title-md font-bold">Failed to load preview</h2>
          <p className="font-body-sm text-body-sm text-on-surface-variant mt-1">{error || "Property not found"}</p>
          <button onClick={() => router.push("/owner/properties")} className="mt-4 px-4 py-2 rounded-lg bg-[#157375] hover:bg-[#0f4a4c] text-white border border-[#157375] text-sm font-medium shadow-sm">Back to properties</button>
        </div>
      </div>
    );
  }
        ...
        <main className="relative pt-6 w-full px-space-lg pb-space-xl bg-surface min-h-screen">
          <div className="flex flex-col w-full">
              ...
              <div className="relative z-10 flex flex-col xl:flex-row xl:items-center justify-between gap-space-md">
                <div className="flex items-start md:items-center gap-space-sm">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="font-body-md text-body-md text-on-primary-container">Interactive guest booking mechanics are simulated. Property ID: {property.id}</p>
                  </div>
                </div>
                <div className="flex items-center gap-space-xs shrink-0 flex-wrap">
                  <Link href={`/owner/properties/${property.id}/edit`} className="px-space-md py-space-xs rounded-xl bg-[#157375] hover:bg-[#0f4a4c] hover:text-white text-white border border-[#157375]/10 transition-all font-label-md text-label-md font-semibold flex items-center gap-1 shadow-sm"><Icon name="edit" className="material-symbols-outlined text-[18px]" /> Edit Listing</Link>
                  ...
                  <Link href="/owner/properties" className="px-space-sm py-space-xs rounded-xl text-white/70 hover:text-white hover:bg-[#157375] transition-all font-label-md text-label-md flex items-center gap-0.5"><Icon name="arrow_back" className="material-symbols-outlined text-[18px]" /> Properties</Link>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-space-xs mb-space-md">
              <div className="flex flex-col">
                <nav className="flex items-center gap-1 font-caption text-caption text-on-surface-variant mb-space-xxs"><span>Dashboard</span><Icon name="chevron_right" className="material-symbols-outlined text-[14px]" /><span>My Properties</span><Icon name="chevron_right" className="material-symbols-outlined text-[14px]" /><span className="text-[#157375] font-semibold truncate max-w-[160px]">{property.title}</span><Icon name="chevron_right" className="material-symbols-outlined text-[14px]" /><span className="text-primary font-bold">Public Preview</span></nav>
                <div className="flex items-baseline gap-space-sm flex-wrap"><h1 className="font-headline-lg text-headline-lg text-[#157375] font-bold">Owner Preview — {property.title}</h1><span className="text-on-surface-variant font-caption text-caption">ID: STY-LB-{property.id}</span></div>
              ...
              <div className="flex items-center gap-space-xs self-start md:self-auto">
                <span className="inline-flex items-center gap-1 px-space-sm py-1 rounded-full bg-surface-container-high text-[#157375] font-caption text-caption font-semibold"><span className="w-2 h-2 rounded-full bg-secondary" /> {property.status === "approved" ? "Instant Booking Enabled" : "Preview Only"}</span>
            ...
            <div className="bg-surface-container-lowest rounded-2xl shadow-sm p-space-md md:p-space-lg">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-space-sm pb-space-md">
                <div className="flex flex-col">
                  ...
                  <h2 className="font-headline-md text-headline-md text-[#157375] font-bold">{property.title}</h2>
                ...
                <div className="flex items-center gap-space-xs">
                  <button disabled className="p-space-xs rounded-xl bg-surface-container-low text-[#157375] font-label-md text-label-md flex items-center gap-1 opacity-60"><Icon name="share" className="material-symbols-outlined text-[18px]" /> Share</button>
                  <span className="px-3 py-1.5 rounded-xl bg-surface-container-high text-[#157375] font-caption text-caption font-semibold">ID #{property.id}</span>
                </div>
              </div>
                ...
                {otherImages.length > 0 ? (
                  <>
                    <div className="md:col-span-2 md:row-span-2 relative group overflow-hidden bg-surface-container">
                      ...
                      <span className="absolute bottom-3 left-3 px-space-xs py-1 rounded-md bg-surface-container-lowest/90 backdrop-blur-md font-caption text-caption text-[#157375] font-semibold">Primary Cover {otherImages[0].is_primary ? "· Primary" : ""}</span>
                    </div>
                    {otherImages.slice(1, 4).map((img, idx) => (
                      <div key={idx} className="relative group overflow-hidden bg-surface-container"><img src={img.image_url} alt={`Property ${idx + 2}`} className="w-full h-full object-cover" /><span className="absolute bottom-2 left-2 px-space-xxs py-0.5 rounded bg-surface-container-lowest/80 text-[#157375] font-caption text-caption">{img.is_primary ? "Primary" : `Photo ${idx + 2}`}</span></div>
                    ))}
                    <div className="relative group overflow-hidden bg-surface-container">
                      ...
                      <span className="absolute bottom-3 right-3 px-space-sm py-1.5 rounded-xl bg-surface-container-lowest text-[#157375] font-label-md text-label-md font-semibold shadow-md">View All {property.images.length} Photos</span>
                    </div>
                  </>
                ) : (
                  <div className="md:col-span-4 md:row-span-2 flex items-center justify-center bg-surface-container text-on-surface-variant flex-col gap-2">
                    ...
                    <Link href={`/owner/properties/${property.id}/images`} className="px-3 py-1.5 rounded-lg bg-[#157375] hover:bg-[#0f4a4c] text-white border border-[#157375] text-sm font-medium shadow-sm">Manage Images</Link>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 gap-space-lg items-start">
                <div className="w-full flex flex-col gap-space-lg">
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-space-sm p-space-md bg-surface-container-low rounded-xl">
                    <div className="flex flex-col"><span className="font-caption text-caption text-on-surface-variant uppercase tracking-wider">Bedrooms</span><span className="font-title-md text-title-md font-bold text-[#157375]">{property.bedrooms} Rooms</span></div>
                    <div className="flex flex-col"><span className="font-caption text-caption text-on-surface-variant uppercase tracking-wider">Beds</span><span className="font-title-md text-title-md font-bold text-[#157375]">{property.beds} Beds</span></div>
                    <div className="flex flex-col"><span className="font-caption text-caption text-on-surface-variant uppercase tracking-wider">Bathrooms</span><span className="font-title-md text-title-md font-bold text-[#157375]">{property.bathrooms} Full</span></div>
                    <div className="flex flex-col"><span className="font-caption text-caption text-on-surface-variant uppercase tracking-wider">Capacity</span><span className="font-title-md text-title-md font-bold text-[#157375]">{property.max_guests} Guests Max</span></div>
                  ...
                  <div className="flex flex-col gap-space-xs">
                    <h3 className="font-headline-sm text-headline-sm font-bold">About this {property.property_type.replace("_", " ")}</h3>
                  ...
                  <div className="flex flex-col gap-space-sm pt-space-xs">
                    <div className="flex items-center justify-between"><h3 className="font-headline-sm text-headline-sm font-bold text-[#157375]">Amenities & Features</h3><span className="font-caption text-caption text-white bg-[#157375] px-2.5 py-0.5 rounded-full font-semibold border border-[#157375]">{property.amenities.length} Verified Features</span></div>
                    {property.amenities.length ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-space-sm">
                        {property.amenities.map((a) => (
                          <div key={a.id} className="flex items-center gap-3 p-3 bg-white border border-[#157375]/10 rounded-xl shadow-sm hover:border-[#157375]/20 hover:shadow-md transition-all"><div className="w-8 h-8 rounded-full bg-[#157375] flex items-center justify-center shrink-0"><Icon name="check_circle" className="material-symbols-outlined text-white text-[18px]" /></div><span className="font-label-md text-label-md font-semibold text-[#157375]">{a.name}</span></div>
                        ))}
                      </div>
                    ) : (
                      <p className="font-body-sm text-body-sm text-[#157375]/70">No amenities selected. <Link href={`/owner/properties/${property.id}/amenities`} className="text-[#157375] hover:underline font-semibold">Configure amenities</Link></p>
                    )}
                  </div>

                  <div className="flex flex-col gap-space-sm pt-space-xs">
                    <h3 className="font-title-md text-title-md font-bold text-[#157375]">House Rules & Policies</h3>
                    {property.property_rules.length ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-space-sm">
                        {property.property_rules.map((pr) => {
                          ...
                          return (
                          <div key={pr.id} className="p-4 bg-white border border-[#157375]/10 rounded-xl flex items-start gap-3 shadow-sm hover:border-[#157375]/20 hover:shadow-md transition-all">
                            ...
                            <div className="flex flex-col"><span className="font-label-md text-label-md font-semibold text-[#157375]">{title} · <span className={pr.allowed ? "text-[#059669]" : "text-[#E11D48]"}>{pr.allowed ? "Allowed" : "Not Allowed"}</span></span>{pr.value && <span className="font-caption text-caption font-medium text-[#157375] bg-[#157375]/5 px-2 py-0.5 rounded-full w-fit mt-1">{pr.value}</span>}</div>
                          </div>
                        )})}
                      </div>
                    ) : (
                      <p className="font-body-sm text-body-sm text-[#157375]/70">No rules configured.</p>
                    )}
                  </div>

                  <div className="flex flex-col gap-space-sm pt-space-xs">
                    <h3 className="font-title-md text-title-md font-bold text-[#157375]">Seasonal Pricing</h3>
                    {property.seasonal_prices.length ? (
                      <div className="flex flex-col gap-2">
                        {property.seasonal_prices.map((s) => (
                          <div key={s.id} className="p-space-sm bg-white border border-[#157375]/10 rounded-xl flex items-center justify-between shadow-sm">
                            ...
                            <span className="font-title-sm text-title-sm font-bold text-[#157375]">${Number(s.price_per_night).toFixed(2)}/night</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="font-body-sm text-body-sm text-[#157375]/70">No seasonal pricing. Base price ${Number(property.price_per_night).toFixed(2)}/night applies year-round. <Link href={`/owner/properties/${property.id}/pricing`} className="text-[#157375] hover:underline font-semibold">Manage pricing</Link></p>
                    )}
                  </div>
                </div>
